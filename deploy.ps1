...
# add access to main and internal resource groups
...
$aksoidcIssuer="$(az aks show --resource-group $resourceGroupName --name $aksName --query "oidcIssuerProfile.issuerUrl" -o tsv)"
$federatedIdentityName="aksfederatedidentity" # can be changed as needed
az identity federated-credential create --name $federatedIdentityName --identity-name 'azurekeyvaultsecretsprovider-cloud-h2t-aks' --resource-group 'MC_cloud-h2t-rg_cloud-h2t-aks_eastus2' --issuer $aksoidcIssuer --subject 'system:serviceaccount:default:workload-identity-sa'
...
#check
kubectl get pods
kubectl exec azure-vote-front-bf9f4f979-qf6h2 -- ls /mnt/secrets-store/